...
glass[, seq(3, {length(glass) - 1})] <- apply(glass[, seq(3, {length(glass) - 1})], 2, FUN="scale")
```
</p>
...
hyperknn <- train(Type ~ ., data=scaledData$Original, method='kknn', trControl = trCtrl, tuneGrid = data.frame(kmax = rep(c(3:20),10),
           distance = rep(rep(1,18),10),
           kernel = c(rep('rectangular',18),rep("triangular",18), rep("epanechnikov",18),rep("biweight",18),              rep("triweight",18),rep("cos",18), rep("inv",18), rep("gaussian",18), rep("rank",18), rep("optimal",18)),stringsAsFactors = F
           ))
...
ud <- u[-c(1:2,14,15,17)] # Remove id, date, zipcode, lat, long
od <- hdata[-c(1:2,14,15,17)]

toScale <- rbind(ud,od) # Combine the two sets for setting classes and normalizing
toScale <- apply(toScale,2,scale) # Scale
ud <- as.data.frame(toScale[1, ]) #Unknown
od <- as.data.frame(toScale[-1, ]) #Original Data
trainIndex <- createDataPartition(od$price,times = 5,p = .8)
trCtrl <- caret::trainControl(method="repeatedcv", number=10, repeats = 4,index = trainIndex)

#Create new method for Caret to use knn.reg
knn.reg.caret <- list(type = "Regression",
              library = "FNN",
              loop = NULL)
prm <- data.frame(parameter = c("k", "algorithm"),
                  class = c("numeric", "character"),
                  label = c("k-Neighbors", "Algorithm"))
knn.reg.caret$prm <- prm
knn.reg.Grid <- function(x, y, len = NULL, search = "grid") {
  ## To use grid search:
  if(search == "grid") {
    ...
    out <- data.frame(k = sample(c(1:20),20,replace=T),
                      algorithm = sample(c("kd_tree", "cover_tree", "brute"),20,replace=T))
  }
out
}
knn.reg.caret$grid <- knn.reg.Grid
# ------------------- Thu Feb 08 22:47:25 2018 --------------------#
#TODO http://topepo.github.io/caret/using-your-own-model-in-train.html

hyperknn <- train(price ~ ., data=scaledData$Original, method='kknn', trControl = trCtrl, tuneGrid = data.frame(kmax = rep(c(3:20),10),
           distance = rep(rep(1,18),10),
           kernel = c(rep('rectangular',18),rep("triangular",18), rep("epanechnikov",18),rep("biweight",18),              rep("triweight",18),rep("cos",18), rep("inv",18), rep("gaussian",18), rep("rank",18), rep("optimal",18)),stringsAsFactors = F
           ))
hyperknnPred <- predict(hyperknn,newdata = glassTest)
