...
# According to this site http://a-little-book-of-r-for-time-series.readthedocs.io/en/latest/src/timeseries.html#holt-s-exponential-smoothing.

plot(HoltWinters(ts(data=ots$OccupancyRate,start=min(ots$Period),end=max(ots$Period)), gamma=FALSE))
predict(HoltWinters(ts(data=ots$OccupancyRate,start=min(ots$Period),end=max(ots$Period)), gamma=FALSE))
```
HoltsWinters seems to be ideal for this type of data as it has a "seasonal" flux. This is the most accurate model that allows for forecast predictions to be made. While we were able to create a fairly accurate linear model using sin and cos interaction terms with the addition of the difference in loess smoothing values to the prediction values lying in the upper quartile, I am unaware of a way to made this into a model with which a prediction can be made. 
</p>
