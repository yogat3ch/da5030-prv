...
<p class='a'>
Model ensembles are combinations of learning algorithm models that can be both homogenous (using multiple iterations of the same model) or heterogenous (using multiple complementary models in a an ensemble model). Ensembles serve to reduce variance (bagging), bias (boosting), or improve prediction (stacking).<sup>2</sup> Ensembles models are more accurate because they aggregate multiple predictive inputs rather than a single predictive model.  Classifications are typically made through voting (bagging), or weighted voting (boosting) methods, where continuous features (regression tasks) are concerned predictions are made through taking the mean or median (bagging) or weighted mean (boosting).
<strong>Bagging</strong>
Bagging is short for bootstrap aggregating where each model is trained on a random sampling with replacement of the dataset. Each model produced has an equal weight in predicting the outcome, a key difference between bagging and boosting. By averaging or voting the predictive outputs of multiple predictive models made from data that was sampled with replacement (bootstrapped) the variance in the data is better accounted for and more accurate predictions can be made. Bagging can be run in parallel because the models are created independently of one another. Model types where data perturbation can produce major model changes (like decision trees) benefit moreso from bagging than do stable learners like kNN where changes in input data do not substantially effect the predictive output. <sup>2</sup>
<strong>Boosting</strong>
Boosting is a sequential model creation method that associates weights to misclassified outputs from previous models such that the next model will emphasize this partition of the data when building the subsequent model to reduce error. Error rates are used to determine each models overall weight in the final predictive model. Boosting will generally result in a more accurate model because of the weighting emphasis placed on misclassified outputs to reduce error. Furthermore, boosting reduces the decision tree tendency to overemphasizes attributes with many levels through the iterative weighting process described above. However, boosting can lead to overfitting, thus, if a model proves to be over-fitted, switching to bagging could resolve the issue.

References
1. XRISTICA. (2018). What is the difference between Bagging and Boosting? - Quantdare. [online] Available at: https://quantdare.com/what-is-the-difference-between-bagging-and-boosting/ [Accessed 28 Feb. 2018].

2. Smolyakov, V. (2018). Ensemble Learning to Improve Machine Learning Results. [online] Stats and Bots. Available at: https://blog.statsbot.co/ensemble-learning-d1dcd548e936 [Accessed 28 Feb. 2018].

3. SeattleDataGuy. (2018). Boosting and Bagging: How To Develop A Robust Machine Learning Algorithm. [online] Available at: https://towardsdatascience.com/how-to-develop-a-robust-algorithm-c38e08f32201 [Accessed 28 Feb. 2018].

4. Kelleher, J., Namee, B. and D'Arcy, A. (2015). Fundamentals of Machine Learning for Predictive Data Analytics. [online] MIT Press. Available at: https://mitpress.mit.edu/books/fundamentals-machine-learning-predictive-data-analytics [Accessed 28 Feb. 2018].
</p>